...
namespace DotNet.DAL.Base
{
    ...
    {
        private readonly string _dbMark = ConfigurationManager.AppSettings["dbMark"];
        /// <summary>
        /// 添加一条记录
        /// </summary>
        ...
        public virtual int Add(T entity)
        {
            ...
            #endregion
            string sqlStr = "insert into dbo.[" + entity.GetType().Name + "] (";
            sqlStr += fields.Substring(0, fields.Length - 1) + ")";
            sqlStr += " values (";
            sqlStr += placeholders.Substring(0, placeholders.Length - 1) + ")";
            //sqlStr.Append(";SELECT @@IDENTITY");   //返回插入用户的主键
            return SqlHelperFactory.GetSqlHelper().ExecuteNonQuery(sqlStr, parameters);
        }
        ...
        /// <returns>整数值</returns>
        public virtual double GetMax(string columnName)
        {
            double number;
            T entity = new T();
            IList<DbParameter> parameters = new List<DbParameter>();
            parameters.Add(SqlHelperFactory.GetSqlHelper().CreateDbParameter(_dbMark + "columnName", columnName));
            string commandText = string.Format("select MAX(" + _dbMark + "columnName) from dbo.[{0}]", entity.GetType().Name);
        ...
        /// <returns></returns>
        public virtual bool Exists(object primarykeyValue)
        {
            #region 参数
            T entity = new T();
            string primaryKeyName = GetPrimarykeyByTableName(entity.GetType().Name);
            IList<DbParameter> parameters = new List<DbParameter>();
            parameters.Add(SqlHelperFactory.GetSqlHelper().CreateDbParameter(_dbMark + primaryKeyName, primarykeyValue));
            #endregion
            string commandText = string.Format("select count(1) from dbo.[{0}] where {1}={2}{1}", entity.GetType().Name, primaryKeyName, _dbMark);
        ...
        public virtual bool Update(T entity)
        {
            ...
            sqlStr.Append("update dbo.[" + entity.GetType().Name);
            sqlStr.Append("] set ");
            sqlStr.Append(fields.Substring(0, fields.Length - 1));
            sqlStr.Append(" where ");
            sqlStr.Append(primaryKeyName + "=" + _dbMark + primaryKeyName);
            return SqlHelperFactory.GetSqlHelper().ExecuteNonQuery(sqlStr.ToString(), parameters) > 0;
        }
        ...
        public virtual bool Delete(object primarykeyValue)
        {
            #region 参数
            T entity = new T();
            string primaryKeyName = GetPrimarykeyByTableName(entity.GetType().Name.ToString());
            IList<DbParameter> parameters = new List<DbParameter>();
            parameters.Add(SqlHelperFactory.GetSqlHelper().CreateDbParameter(_dbMark + primaryKeyName, primarykeyValue));
            #endregion
            StringBuilder sqlStr = new StringBuilder();
            sqlStr.AppendFormat("delete from dbo.[{0}] where ", entity.GetType().Name);
            sqlStr.Append(primaryKeyName + "=" + _dbMark + primaryKeyName);
            return SqlHelperFactory.GetSqlHelper().ExecuteNonQuery(sqlStr.ToString(), parameters) > 0;
        }
        ...
        /// <returns>成功为True</returns>
        public virtual bool DeleteList(IList<object> primarykeyValues)
        {
            #region 参数
            T entity = new T();
            string primaryKeyName = GetPrimarykeyByTableName(entity.GetType().Name);
            string primaryKeys = string.Join(",", primarykeyValues);
            IList<DbParameter> parameters = new List<DbParameter>();
            parameters.Add(SqlHelperFactory.GetSqlHelper().CreateDbParameter(_dbMark + primaryKeyName, primaryKeys));
            #endregion
            StringBuilder sqlStr = new StringBuilder();
        ...
        public abstract DataTable GetTablePaged(int pageIndex, int pageSize, out int totalCount, string strWhere, Dictionary<string, SqlSortEnum> orderBy);
        ...
        public abstract string GetPrimarykeyByTableName(string tableName);
